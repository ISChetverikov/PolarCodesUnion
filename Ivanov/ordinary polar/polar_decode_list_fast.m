function [u,x,idx,listprob] = polar_decode_list_fast(y,f,L,listprob,u)
    ...
    if (N==1)
     ...
     elseif f{1} == 0 && all(cell2mat(f(2:end)) == 1/2)
        ...
        [uhat1,u1hardprev,idx1,prob1] = polar_decode_list_fast(u1est,f(1:(N/2)),L,listprob,u);
        ...
        [uhat2,u2hardprev,idx2,prob2] = polar_decode_list_fast(u2est,f((N/2+1):end),L,prob1,uhat1);
...
function [u,x,idx,listprob]=decRate1(y,L,listprob,u)
    u_hat=kron(y<0,ones(4,1));
    %logp=-log1p(exp(-abs(y)));
    logp=zeros(1,length(y));
